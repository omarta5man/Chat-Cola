...
function FeatureCard({ feature, index }: { feature: { icon: any; titleKey: string; subtitleKey: string; descriptionKey: string; accent: string }; index: number }) {
  ...
  return (
    ...
    >
      ...
      <div className="relative bg-[#1a1a1a] rounded-2xl p-5 border border-white/10 overflow-hidden h-full">
        ...
        {/* Content */}
        <div className={`relative z-10 flex flex-col h-full min-h-[140px] ${isRTL ? "text-right" : "text-left"}`}>
          {/* Icon with pulse animation */}
          <motion.div
            className={`w-10 h-10 rounded-xl flex items-center justify-center mb-4 relative ${isRTL ? "ml-auto" : ""}`}
          ...
          <div className="flex-1">
            <motion.div
              className={`text-3xl font-black tracking-tight text-white ${isRTL ? "font-arabic" : ""}`}
              initial={{ opacity: 0, scale: 0.5 }}
              whileInView={{ opacity: 1, scale: 1 }}
              viewport={{ once: true }}
              transition={{ type: "spring", stiffness: 200, damping: 20, delay: 0.2 + index * 0.1 }}
            >
              <span style={{ color: feature.accent }}>{t(feature.titleKey)}</span>
            </motion.div>
            <h3 className={`text-sm font-semibold text-white mt-1 ${isRTL ? "font-arabic" : ""}`}>{t(feature.subtitleKey)}</h3>
            <p className={`text-xs text-white/50 mt-1 ${isRTL ? "font-arabic leading-relaxed" : "font-mono"}`}>{t(feature.descriptionKey)}</p>
          </div>
...
export function BentoGrid() {
  ...
  return (
    <section id="formula" className="relative py-16 bg-[#121212] overflow-hidden" dir={isRTL ? "rtl" : "ltr"}>
      ...
      <div ref={ref} className="max-w-5xl mx-auto px-6 relative z-10">
        ...
        >
          ...
          <div className="overflow-hidden mt-2">
            <motion.h2
              className={`text-3xl md:text-4xl font-black text-white tracking-tight ${isRTL ? "font-arabic" : ""}`}
              initial={{ y: 60 }}
          ...
          {/* Animated underline */}
          <motion.div
            className="h-[2px] w-12 bg-red-600 mx-auto mt-3 rounded-full"
            initial={{ scaleX: 0 }}
